...
void lcd_status_screen() {
    ...
    if (lcd_clicked) {
      ...
      //lcd_goto_screen(lcd_main_menu);//Kalle
			lcd_goto_screen(lcd_prepare_menu, ENCODER_STEPS_PER_MENU_ITEM); //Kalle
      return;
    }
...
void kill_screen(const char* lcd_msg) {
  ...
  static void lcd_setZProbeHeight_menu() {
    ...
    if (encoderPosition) { 
      ...
      if(home_offset[Z_AXIS] + float((int32_t)encoderPosition) * 0.01/ENCODER_STEPS_PER_MENU_ITEM >= 0){ //Only allow positive offset
   ...
   */
   static void HomeAll() {
     ...
     enqueue_and_echo_commands_P(PSTR("G1 F300 Z10"));
   ...
   static void lcd_GoToHome_menu() {
	...
	static void EndstopHomeXY(){
		enqueue_and_echo_commands_P(PSTR("G28 X Y"));
		enqueue_and_echo_commands_P(PSTR("G1 F3000 X200 Y200"));
    enqueue_and_echo_commands_P(PSTR("G92 X0 Y0 Z0")); //Make this home
	}
	static void EndstopHomeX(){
		enqueue_and_echo_commands_P(PSTR("G28 X"));
		enqueue_and_echo_commands_P(PSTR("G1 F3000 X200 Y200"));
    enqueue_and_echo_commands_P(PSTR("G92 X0")); //Make this X home
	}
	static void EndstopHomeY(){
		enqueue_and_echo_commands_P(PSTR("G28 Y"));
		enqueue_and_echo_commands_P(PSTR("G1 F3000 X200 Y200"));
    enqueue_and_echo_commands_P(PSTR("G92 Y0")); //Make this Y home
	}
 ...
 static void lcd_SetXYHome_menu() {
    ...
    MENU_ITEM(back, MSG_MAIN,0);
		...
		MENU_ITEM(function, "X and Y", EndstopHomeXY); 
		MENU_ITEM(function, "X", EndstopHomeX); 
		MENU_ITEM(function, "Y", EndstopHomeY); 
    //MENU_ITEM(gcode, "X and Y", PSTR("G28 X Y")); //All (not Z)
   ...
   *
   -Startscreen
    -Back
    -Move Axis
      -0.1mm (Z,X,Y) option set coordiates first -then execute "G1"
      -1mm (Z,X,Y)
      -10mm (X,Y)
    -Probe for Z0
    -Zero only Z
    ...
    MENU_ITEM(back, MSG_MAIN,0);
        ...
        if (card.cardOK) {
			...
			//SD-card: 
				...
				MENU_ITEM(submenu2, MSG_NO_CARD, lcd_sdcard_menu);
				#if !PIN_EXISTS(SD_DETECT)
					MENU_ITEM(gcode, "Mount card", PSTR("M21")); // Manually initialize the SD-card via user interface
				#endif
				}
			#endif //SDSUPPORT
  ...
  void _lcd_move_xyz(const char* name, AxisEnum axis) {
    ...
    if (encoderPosition%ENCODER_STEPS_PER_MENU_ITEM==0){ //Kalle to make one knob-step equal one movescale! if(encoderposition){
      refresh_cmd_timeout();
    ...
    //}
      ...
      encoderPosition = 0;
      lcdDrawUpdate = LCDVIEW_REDRAW_NOW; //Kalle kanske ska flyttas utanför if-satsen... men känns onödigt!
    }
    if (lcdDrawUpdate)
  ...
  static void SetCoordinateWithMultiplier(const char* namn, AxisEnum axis) {
    ...
    if (encoderPosition) {
      ...
      if(encoderPosition%ENCODER_STEPS_PER_MENU_ITEM==0){ //Kalle to make one knob-step equal one movescale!
        float Addition = float((int32_t)encoderPosition)/ENCODER_STEPS_PER_MENU_ITEM;//Kalle ENCODER_STEPS_PER_MENU_ITEM=4
        if(axis == X_AXIS){
    ...
    //if (lcdDrawUpdate){
     if(axis == X_AXIS){
        ...
        lcd_implementation_drawedit(namn, ftostr41sign(X_Coordinate_Move));
      }else if(axis == Y_AXIS){
        lcd_implementation_drawedit(namn, ftostr41sign(Y_Coordinate_Move));
      }else{
        lcd_implementation_drawedit(namn, ftostr41sign(Z_Coordinate_Move));
      }
  ...
  void _lcd_move_distance_menu(const AxisEnum axis, const screenFunc_t func) {
    ...
    MENU_BACK(MSG_MOVE_AXIS);
		MENU_ITEM(submenu, MSG_MOVE_01MM, lcd_move_menu_01mm); //Rearange order Kalle
  ...
  void menu_action_submenu(screenFunc_t func) { lcd_save_previous_screen(); lcd_goto_screen(func); }
	static void menu_action_submenu2(screenFunc_t func) { lcd_save_previous_screen(); lcd_goto_screen(func,ENCODER_STEPS_PER_MENU_ITEM); } //Kalle jump down ENCODER_STEPS_PER_MENU_ITEM ("5") encoder steps = one menu item
  
  void menu_action_gcode(const char* pgcode) { enqueue_and_echo_commands_P(pgcode); }
  void menu_action_function(screenFunc_t func) { (*func)(); }
  const char* Selectedfilename; //Kalle
  const char *SelLongFilename; //Kalle
	static void FileConfirmedToRun(){ //Kalle
    //UNUSED(longFilename);
    card.openAndPrintFile(Selectedfilename);
    lcd_return_to_status();
  }
  static void confirmRunFile(){//Kalle
    START_MENU();
    MENU_ITEM(back,"CANCEL",0);
    ...
    void menu_action_sdfile(const char* filename, char* longFilename) {
			Selectedfilename = filename;//Kalle*6
